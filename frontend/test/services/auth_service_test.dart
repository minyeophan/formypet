...
void main() {
  ...
  test(
    'updateProfile sends PATCH nickname and parses the returned id',
    () async {
  ...
  test(
    'uploadProfileImage sends a multipart file with its original name',
    () async {
      FormData? captured;
      dio.httpClientAdapter = _CannedAdapter((options) {
  ...
  test('deleteAccount sends the local password to the authenticated account endpoint', () async {
    RequestOptions? captured;
    dio.httpClientAdapter = _CannedAdapter((options) {
      captured = options;
      return _jsonResponse(options, 202, {
        'data': {'status': 'ACCEPTED', 'externalCleanupPending': false},
      });
    });

    final accepted = await AuthService().deleteAccount(password: 'Password1!');

    expect(accepted, isTrue);
    expect(captured?.method, 'DELETE');
    expect(captured?.path, '/api/v1/users/me');
    expect(captured?.data, {'password': 'Password1!'});
  });
}
