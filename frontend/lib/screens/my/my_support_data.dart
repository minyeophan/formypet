...
const myFaqs = [
  ...
  MyFaq(
    ...
    body:
        '마이페이지의 1대1 문의하기에서 유형, 답변받을 이메일, 제목과 내용을 입력해 주세요.\n\n접수 완료 안내와 접수번호가 표시되면 문의가 저장된 상태예요. 운영자가 확인한 뒤 입력한 이메일로 답변드려요. 접수 실패 안내가 나오면 작성 내용은 유지되며 다시 시도할 수 있어요.',
  ),
  MyFaq(
    id: 'community-report',
    categoryId: 'community',
    ...
    body:
        '다른 사용자의 게시글 상세에서 더보기 메뉴의 신고하기를 선택해 주세요. 사유를 선택하고 필요한 내용을 적어 접수할 수 있어요. 기타 사유는 상세 내용이 필요해요.\n\n접수번호가 표시되면 신고가 저장된 상태예요. 신고만으로 게시글이 바로 삭제되거나 작성자가 차단되지는 않아요. 본인 게시글과 댓글·답글에는 신고 버튼을 제공하지 않아요.',
  ),
  MyFaq(
    id: 'community-block',
    categoryId: 'community',
    ...
    body:
        '다른 사용자의 게시글 상세에서 더보기 메뉴의 작성자 차단을 선택할 수 있어요. 마이페이지 또는 설정의 차단 목록에서 해제할 수 있어요.\n\n차단한 작성자의 게시글과 알림은 내 화면에서 숨겨지고 댓글은 내용 없는 안내로 표시돼요. 다른 사용자의 답글은 유지돼요. 차단을 해제하면 기존 콘텐츠를 다시 볼 수 있어요. 처리 실패 시에는 다시 시도해 주세요.',
  ),
  MyFaq(
    id: 'community-news',
    categoryId: 'community',
