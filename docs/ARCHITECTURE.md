...
│   │   └── record_support.dart          # 기록 화면 공통 지원
│   ├── community/
...
- **기록 화면 경계**: 현재 Home과 라우터는 통합 기록 화면과 `widgets/record_inputs/` 공통 입력 패널을 사용한다. 활동 기록 타입은 9개만 지원하며 제거된 `play`, `sleep`, `checkup`, `bath`, `groom`의 입력 분기는 보유하지 않는다.
