...
- 단일 테스트 클래스 전용 Testcontainers 컨테이너는 `@Container`와 `static` 필드로 선언해 클래스 내에서 재사용한다.
- 여러 통합 테스트 클래스가 공유하는 컨테이너는 전용 owner class의 JVM singleton으로만 관리한다. 종료는 Testcontainers Ryuk에 맡기고 별도 shutdown hook이나 experimental reusable container를 추가하지 않는다.
...
- `maxParallelForks = 1`과 기본 `forkEvery = 0`을 유지한다. worker 수나 fork 정책을 바꾸면 JVM singleton 범위와 DB 격리 정책을 다시 검토한다.
