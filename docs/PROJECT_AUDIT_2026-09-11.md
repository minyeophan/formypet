...
| F-05 | 1대1 문의 — 화면·호출 구조 구현 | 기존 문의 화면에서 유형·답변 이메일·제목·내용 입력, 검사·접수 중·실패 재시도·완료 표시 제공 | DB 접수·운영자 이메일 전달은 서버 연동 단계. API 미제공 시 실패 표시. 앱 내 문의함·답변 화면 제외 |
...
| ID | 상태 | 내용 및 근거 |
|---|---|---|
| O-01 | 정책 원문 미확정 | 정책 상세에 항목 소개이며 확정 정책이 아님을 표시하고 서비스명을 포마펫으로 정리했다. 정식 원문은 운영 정보 확인 후 교체 필요. 확인 항목은 `docs/SUPPORT_CONTENT.md` 참고 |
| O-02 | 고객지원 안내 정리 | 샘플 공지 3개 제거 및 빈 상태 제공. FAQ를 실제 기록·루틴·투표 동작과 신고·차단·문의 연결 준비 상태에 맞춰 갱신했다. 공지/FAQ는 정적 데이터이며 서버 관리 API는 별도 범위 |
