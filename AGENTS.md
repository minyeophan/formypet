...
- 브랜치 역할은 `main`(배포 기준), `develop`(통합 기준), `feature/*`·`fix/*`(개별 작업)로 구분하고 기본 병합 방향은 `feature/*`·`fix/* -> develop -> main`으로 한다.
- AI는 사용자 요청 없이 브랜치를 생성·전환·병합하지 않는다.
- 작업 시작과 종료 시 `git status --short`로 변경 범위만 확인한다.
- 변경 이유 확인이 필요할 때만 `git diff -- <path>`를 사용한다.
- 원격, 브랜치, 로그 조회는 사용자 요청이 있거나 작업상 꼭 필요할 때만 한다.
- rebase, `reset --hard`, 강제 push처럼 기존 히스토리를 바꾸는 작업은 명시적 요청 없이는 수행하지 않는다.
