...
지도 검색·GPS 위치 수집은 제공하지 않습니다. 일정 장소와 병원명은 직접 입력하며, 산책 거리·시간·메모와 병원 기록·사진·루틴 연결은 유지됩니다. 카카오 로그인 설정은 지도 기능과 별개이며 유지합니다.

기록 및 루틴 API는 이전 클라이언트가 보내는 `startLng`, `startLat`, `endLng`, `endLat`, `clinicLng`, `clinicLat`를 저장하지 않고 응답에서도 제외합니다. 루틴에서는 최상위 좌표 키만 제거하며 다른 상세 값과 상세 입력 생략 시 기존 값 유지 동작은 보존합니다.

V30은 산책의 시작·종료 좌표 컬럼, 병원 좌표 컬럼, `spatial_test` 테이블을 삭제하고 기존 루틴 상세 JSON의 최상위 좌표 키를 제거합니다. 실제 DB 적용 전에는 다음 순서를 지킵니다.

1. 기존 백엔드 인스턴스를 모두 중지하고 DB 전체 백업 및 복원 가능 여부를 확인합니다.
2. 새 백엔드를 배포한 뒤 Flyway로 V30을 적용합니다. 기존 백엔드는 삭제된 컬럼을 참조하므로 신·구 버전을 동시에 실행하지 않습니다.
3. 기록 조회·저장, 사진과 루틴 연결을 확인합니다. 통합 테스트는 별도 테스트 DB에서 실행하며 실제 DB 적용을 대신하지 않습니다.

삭제된 좌표는 코드 롤백만으로 복구되지 않습니다. 복구가 필요하면 백엔드를 중지한 상태에서 적용 전 DB 백업과 이전 코드 버전을 함께 복원해야 합니다. MySQL DDL은 중간 실패 시 일부 변경이 남을 수 있으므로 무작정 재실행하거나 Flyway 이력만 수정하지 말고 실제 스키마와 백업을 확인합니다.
