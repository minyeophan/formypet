...
| 커뮤니티 | 부분 구현 | 피드·글쓰기·좋아요·상세·이미지·투표·카테고리·댓글·답글 API/UI가 연동되어 있습니다. 커뮤니티 검색은 후속 범위입니다. |
| 지갑 | 부분 구현 | 지갑 요약과 지출 생성·목록·상세·수정·삭제, 기간별 조회가 API/UI에 연동되어 있습니다. 지출 사진 첨부는 후속 범위입니다. |
| 일정 | 부분 구현 | 반려동물 일정 생성·목록·상세·수정·삭제와 루틴 관리가 API/UI에 연동되어 있습니다. 지도 검색과 실제 알림 실행 연결은 후속 범위입니다. |
